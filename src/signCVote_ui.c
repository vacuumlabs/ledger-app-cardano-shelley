...
#ifdef HAVE_BAGL
        ui_displayPrompt("Start new", "vote? (CIP-36)", this_fn, respond_with_user_reject);
#elif defined(HAVE_NBGL)
...
#ifdef HAVE_BAGL
        ui_displayHexBufferScreen("Vote plan id",
                                  ctx->votePlanId,
                                  SIZEOF(ctx->votePlanId),
                                  this_fn);
#elif defined(HAVE_NBGL)
        char bufferHex[2 * VOTE_PLAN_ID_SIZE + 1] = {0};
...
#ifdef HAVE_BAGL
        ui_displayUint64Screen("Proposal index", ctx->proposalIndex, this_fn);
#elif defined(HAVE_NBGL)
        char line[30];
        ui_getUint64Screen(line, SIZEOF(line), ctx->proposalIndex);
        fill_and_display_if_required("Proposal index", line, this_fn, respond_with_user_reject);
#endif  // HAVE_BAGL
    }
    UI_STEP(HANDLE_INIT_PAYLOAD_TYPE_TAG) {
#ifdef HAVE_BAGL
        ui_displayUint64Screen("Payload type tag", ctx->payloadTypeTag, this_fn);
#elif defined(HAVE_NBGL)
        char line[30];
        ui_getUint64Screen(line, SIZEOF(line), ctx->payloadTypeTag);
