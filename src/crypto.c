...
static cx_err_t crypto_init_privkey(
        const uint32_t* path,
        size_t path_len,
        ...
        uint8_t* chain_code)
{
	...
	CX_CHECK(os_derive_bip32_no_throw(CX_CURVE_Ed25519,
	                                  path,
	                                  path_len,
	                                  raw_privkey,
	                                  chain_code));
...
WARN_UNUSED_RESULT cx_err_t crypto_get_pubkey(
        const uint32_t* path,
        size_t path_len,
        ...
        uint8_t* chain_code)
{
	...
	// Derive private key according to BIP32 path
	CX_CHECK(crypto_init_privkey(path,
	                             path_len,
	                             &privkey,
	                             chain_code));
...
WARN_UNUSED_RESULT cx_err_t crypto_eddsa_sign(
        const uint32_t* path,
        size_t path_len,
        const uint8_t* hash,
        size_t hash_len,
