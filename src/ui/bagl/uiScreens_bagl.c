...
void ui_displayBech32Screen(const char* firstLine,
    ...
    {
        size_t len =
            bech32_encode(bech32Prefix, buffer, bufferSize, encodedStr, SIZEOF(encodedStr));
...
void ui_displayHexBufferScreen(const char* firstLine,
    ...
    size_t length = encode_hex(buffer, bufferSize, bufferHex, SIZEOF(bufferHex));
...
void ui_displayPathScreen(const char* firstLine,
    ...
    ui_displayPaginatedText(firstLine, pathStr, callback);
}
...
__noinline_due_to_stack__ static void _ui_displayAccountWithDescriptionScreen(
    ...
    if (showAccountDescription) {
        ...
        if (bip44_hasByronPrefix(path)) {
            snprintf(accountDescription,
                     SIZEOF(accountDescription),
                     "Byron account #%u  ",
                     account + 1);
        } else if (bip44_hasShelleyPrefix(path)) {
            snprintf(accountDescription, SIZEOF(accountDescription), "Account #%u  ", account + 1);
        } else {
            ASSERT(false);
        }
    }
    ...
    ui_displayPaginatedText(firstLine, accountDescription, callback);
}
...
void ui_displayStakingKeyScreen(const bip44_path_t* stakingPath, ui_callback_fn_t callback) {
    ...
    _ui_displayAccountWithDescriptionScreen("Stake key",
                                            stakingPath,
...
void ui_displayAddressScreen(const char* firstLine,
    ...
    size_t length =
        humanReadableAddress(addressBuffer, addressSize, humanAddress, SIZEOF(humanAddress));
    ASSERT(length > 0);
    ASSERT(strlen(humanAddress) == length);

    ui_displayPaginatedText(firstLine, humanAddress, callback);
}
...
static void _displayRewardAccountWithDescriptionScreen(const key_reference_type_t keyReferenceType,
    ...
    ui_displayPaginatedText(firstLine, description, callback);
}
...
void ui_displayRewardAccountScreen(const reward_account_t* rewardAccount,
    ...
    switch (rewardAccount->keyReferenceType) {
        case KEY_REFERENCE_PATH: {
...

            {
                ...
                STATIC_ASSERT(sizeof(account + 1) <= sizeof(unsigned), "oversized type for %u");
                STATIC_ASSERT(!IS_SIGNED(account + 1), "signed type for %u");
                snprintf(firstLine, SIZEOF(firstLine), "Reward account #%u  ", account + 1);
            }
        ...
        case KEY_REFERENCE_HASH: {
            snprintf(firstLine, SIZEOF(firstLine), "Reward account");
    ...
    _displayRewardAccountWithDescriptionScreen(rewardAccount->keyReferenceType,
                                               &rewardAccount->path,
                                               rewardAccountBuffer,
                                               firstLine,
                                               callback);
...
void ui_displayPaymentInfoScreen(const addressParams_t* addressParams, ui_callback_fn_t callback) {
    switch (determinePaymentChoice(addressParams->type)) {
        ...
        case PAYMENT_SCRIPT_HASH: {
            ui_displayBech32Screen("Payment script hash",
                                   "script",
...
void ui_displayValidityBoundaryScreen(const char* firstLine,
    ...
    if ((networkId == MAINNET_NETWORK_ID) && (protocolMagic == MAINNET_PROTOCOL_MAGIC)) {
        ...
        ui_displayPaginatedText(firstLine, boundaryStr, callback);
    } else {
        ui_displayUint64Screen(firstLine, boundary, callback);
    }
}
...
void ui_displayNetworkParamsScreen(const char* firstLine,
    ...
    snprintf(networkParams, SIZEOF(networkParams), "network id %u", networkId);
#endif  // APP_FEATURE_BYRON_PROTOCOL_MAGIC_CHECK
    ASSERT(strlen(networkParams) + 1 < SIZEOF(networkParams));

    ui_displayPaginatedText(firstLine, networkParams, callback);
}

void ui_displayPoolMarginScreen(uint64_t marginNumerator,
    ...
    ui_displayPaginatedText("Profit margin", marginStr, callback);
}
...
void ui_displayPoolOwnerScreen(const pool_owner_t* owner,
    ...
    }
    {
        ...
        _displayRewardAccountWithDescriptionScreen(owner->keyReferenceType,
                                                   &owner->path,
                                                   rewardAddress,
                                                   firstLine,
                                                   callback);
...
void ui_displayIpPortScreen(const ipport_t* port, ui_callback_fn_t callback) {
    ...
    ui_displayPaginatedText("Port", portStr, callback);
}
...
void ui_displayInputScreen(const sign_tx_transaction_input_t* input, ui_callback_fn_t callback) {
    ...
    ui_displayPaginatedText(input->label, inputStr, callback);
}

void fee_high_cb(void) {
    ui_displayAdaAmountScreen("Transaction fee",
                              BODY_CTX->stageData.fee,
