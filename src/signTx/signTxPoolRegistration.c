...
__noinline_due_to_stack__ static void signTxPoolRegistration_handleInitAPDU(
    ...
    }
    {
        ...
        TRACE("num owners, relays: %d %d", subctx->numOwners, subctx->numRelays);
    ...
    }
    {
        security_policy_t policy =
            policyForSignTxStakePoolRegistrationInit(commonTxData->txSigningMode,
                                                     subctx->numOwners);
        TRACE("Policy: %d", (int) policy);
        ENSURE_NOT_DENIED(policy);
    }
    {
        txHashBuilder_poolRegistrationCertificate_enter(&BODY_CTX->txHashBuilder,
                                                        subctx->numOwners,
                                                        subctx->numRelays);
    }
...
static void _parsePoolId(read_view_t* view) {
    ...
    switch (key->keyReferenceType) {
        ...
        case KEY_REFERENCE_PATH: {
            view_skipBytes(view,
                           bip44_parseFromWire(&key->path, VIEW_REMAINING_TO_TUPLE_BUF_SIZE(view)));
...
__noinline_due_to_stack__ static void signTxPoolRegistration_handlePoolKeyAPDU(
...

    {
        ...
        txHashBuilder_poolRegistrationCertificate_poolKeyHash(&BODY_CTX->txHashBuilder,
                                                              poolKeyHash,
                                                              SIZEOF(poolKeyHash));
    }
    {
...
__noinline_due_to_stack__ static void signTxPoolRegistration_handleVrfKeyAPDU(
    ...
    pool_registration_context_t* subctx = accessSubcontext();
    {
        ...
        {
            STATIC_ASSERT(SIZEOF(subctx->stateData.vrfKeyHash) == VRF_KEY_HASH_LENGTH,
                          "wrong vrfKeyHash size");
    ...
    security_policy_t policy =
        policyForSignTxStakePoolRegistrationVrfKey(commonTxData->txSigningMode);
    TRACE("Policy: %d", (int) policy);
    ENSURE_NOT_DENIED(policy);

    {
        txHashBuilder_poolRegistrationCertificate_vrfKeyHash(&BODY_CTX->txHashBuilder,
    ...
    pool_registration_context_t* subctx = accessSubcontext();
    {
...

        {
            ...
            VALIDATE(subctx->stateData.marginDenominator <= MARGIN_DENOMINATOR_MAX,
                     ERR_INVALID_DATA);
            VALIDATE(subctx->stateData.marginNumerator <= subctx->stateData.marginDenominator,
                     ERR_INVALID_DATA);
        }
    }
    {
        txHashBuilder_poolRegistrationCertificate_financials(&BODY_CTX->txHashBuilder,
    ...
    switch (rewardAccount->keyReferenceType) {
        case KEY_REFERENCE_HASH: {
            ...
            const uint8_t header =
                getAddressHeader(rewardAccount->hashBuffer, SIZEOF(rewardAccount->hashBuffer));
            VALIDATE(
                getAddressType(header) == REWARD_KEY || getAddressType(header) == REWARD_SCRIPT,
                ERR_INVALID_DATA);
        ...
        case KEY_REFERENCE_PATH: {
            view_skipBytes(
                view,
                bip44_parseFromWire(&rewardAccount->path, VIEW_REMAINING_TO_TUPLE_BUF_SIZE(view)));
...
__noinline_due_to_stack__ static void signTxPoolRegistration_handleRewardAccountAPDU(
...

    {
        ...
        rewardAccountToBuffer(&subctx->stateData.poolRewardAccount,
                              commonTxData->networkId,
                              rewardAccountBuffer);

        txHashBuilder_poolRegistrationCertificate_rewardAccount(&BODY_CTX->txHashBuilder,
                                                                rewardAccountBuffer,
...
__noinline_due_to_stack__ static void _addOwnerToTxHash() {
    ...
    txHashBuilder_addPoolRegistrationCertificate_addOwner(&BODY_CTX->txHashBuilder,
                                                          ownerKeyHash,
                                                          SIZEOF(ownerKeyHash));
    TRACE();
}
...
__noinline_due_to_stack__ static void signTxPoolRegistration_handleOwnerAPDU(
...

    {
        ...
        switch (owner->keyReferenceType) {
            ...
            case KEY_REFERENCE_PATH: {
                view_skipBytes(
                    &view,
                    bip44_parseFromWire(&owner->path, VIEW_REMAINING_TO_TUPLE_BUF_SIZE(&view)));
    ...
    security_policy_t policy =
        policyForSignTxStakePoolRegistrationOwner(commonTxData->txSigningMode,
                                                  owner,
                                                  subctx->numOwnersGivenByPath);
    TRACE("Policy: %d", (int) policy);
    ENSURE_NOT_DENIED(policy);
...
__noinline_due_to_stack__ static void signTxPoolRegistration_handleRelayAPDU(
    ...
    security_policy_t policy =
        policyForSignTxStakePoolRegistrationRelay(commonTxData->txSigningMode, relay);
    TRACE("Policy: %d", (int) policy);
    ENSURE_NOT_DENIED(policy);
...
#define CASE(POLICY, UI_STEP)                  \
    case POLICY: {                             \
        accessSubcontext()->ui_step = UI_STEP; \
        break;                                 \
    }
...
__noinline_due_to_stack__ static void signTxPoolRegistration_handlePoolMetadataAPDU(
...

    {
        ...
        {
            STATIC_ASSERT(SIZEOF(md->hash) == POOL_METADATA_HASH_LENGTH,
                          "wrong pool metadata buffer size");
        ...
        }
        {
            ...
            STATIC_ASSERT(SIZEOF(md->url) >= POOL_METADATA_URL_LENGTH_MAX,
                          "wrong pool metadata url size");
...

    {
        ...
        txHashBuilder_addPoolRegistrationCertificate_addPoolMetadata(
            ...
            subctx->stateData.metadata.url,
            subctx->stateData.metadata.urlSize,
            subctx->stateData.metadata.hash,
            SIZEOF(subctx->stateData.metadata.hash));
    }
