    ...
    switch (subctx->stateData.delegation.type) {
        case DELEGATION_KEY: {
...
#ifdef HAVE_BAGL
            ui_displayBech32Screen("Vote public key",
                                   "cvote_vk",
            ...
            ui_getBech32Screen(encodedStr,
                               ...
                               subctx->stateData.delegation.votePubKey,
                               CVOTE_PUBLIC_KEY_LENGTH);
            fill_and_display_if_required("Vote public key",
                                         encodedStr,
                                         callback,
...
#ifdef HAVE_BAGL
            ui_displayPathScreen("Vote public key",
...
#ifdef HAVE_BAGL
        ui_displayPaginatedText("WARNING:", "unusual vote key", this_fn);
#elif defined(HAVE_NBGL)
        set_light_confirmation(true);
...
#ifdef HAVE_BAGL
        ui_displayUint64Screen("Weight", subctx->stateData.delegation.weight, this_fn);
#elif defined(HAVE_NBGL)
        set_light_confirmation(true);
        char line[30];
        ui_getUint64Screen(line, SIZEOF(line), subctx->stateData.delegation.weight);
...
void signTxCVoteRegistration_handleStakingKey_ui_runStep() {
    ...
    UI_STEP(HANDLE_STAKING_KEY_STEP_DISPLAY) {
#ifdef HAVE_BAGL
        ui_displayStakingKeyScreen(&subctx->stakingKeyPath, this_fn);
#elif defined(HAVE_NBGL)
        set_light_confirmation(true);
        ...
        if (showAccountDescription) {
            ...
            fill_and_display_if_required(line1,
                                         line2,
                                         signTxCVoteRegistration_handleStakingKey_ui_cb,
                                         respond_with_user_reject);
        }
#endif  // HAVE_BAGL
    }
...
#ifdef HAVE_BAGL
        ui_displayUint64Screen("Nonce", subctx->stateData.nonce, this_fn);
#elif defined(HAVE_NBGL)
        char line[30];
        ui_getUint64Screen(line, SIZEOF(line), subctx->stateData.nonce);
...
#ifdef HAVE_BAGL
        ui_displayUint64Screen("Voting purpose", subctx->stateData.votingPurpose, this_fn);
#elif defined(HAVE_NBGL)
        char line[30];
        ui_getUint64Screen(line, SIZEOF(line), subctx->stateData.votingPurpose);
...
// so we want to show it in non-expert mode too
#ifdef HAVE_BAGL
        ui_displayPrompt("Confirm vote key", "registration?", this_fn, respond_with_user_reject);
#elif defined(HAVE_NBGL)
        display_confirmation("Confirm vote key\nregistration",
                             "",
                             "VOTE KEY\nCONFIRMED",
                             "Vote key\nrejected",
                             this_fn,
    ...
    UI_STEP(HANDLE_CONFIRM_STEP_RESPOND) {
        ...
        STATIC_ASSERT(SIZEOF(subctx->auxDataHash) == AUX_DATA_HASH_LENGTH,
                      "Wrong aux data hash length");
        memmove(wireResponse.auxDataHash, subctx->auxDataHash, AUX_DATA_HASH_LENGTH);

        STATIC_ASSERT(SIZEOF(subctx->stateData.registrationSignature) == ED25519_SIGNATURE_LENGTH,
                      "Wrong CIP-36 voting registration signature length");
        memmove(wireResponse.signature,
