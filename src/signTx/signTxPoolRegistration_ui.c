...
#ifdef HAVE_BAGL
        ui_displayPaginatedText("Pool registration", "certificate", this_fn);
#elif defined(HAVE_NBGL)
        display_prompt("Pool registration\ncertificate", "", this_fn, respond_with_user_reject);
#endif  // HAVE_BAGL
    }
...
#ifdef HAVE_BAGL
        ui_displayPathScreen("Pool ID path", &subctx->stateData.poolId.path, this_fn);
#elif defined(HAVE_NBGL)
        char pathStr[BIP44_PATH_STRING_SIZE_MAX + 1] = {0};
        ui_getPathScreen(pathStr, SIZEOF(pathStr), &subctx->stateData.poolId.path);
        fill_and_display_if_required("Pool ID path", pathStr, this_fn, respond_with_user_reject);
#endif  // HAVE_BAGL
    }
...
#ifdef HAVE_BAGL
        ui_displayBech32Screen("Pool ID", "pool", poolKeyHash, SIZEOF(poolKeyHash), this_fn);
#elif defined(HAVE_NBGL)
...
void handlePoolVrfKey_ui_runStep() {
    ...
    UI_STEP(HANDLE_POOL_VRF_KEY_STEP_DISPLAY) {
#ifdef HAVE_BAGL
        ui_displayBech32Screen("VRF key hash",
                               "vrf_vk",
                               subctx->stateData.vrfKeyHash,
                               SIZEOF(subctx->stateData.vrfKeyHash),
                               this_fn);
#elif defined(HAVE_NBGL)
        char encodedStr[BECH32_STRING_SIZE_MAX] = {0};
        ui_getBech32Screen(encodedStr,
        ...
        fill_and_display_if_required("VRF key hash", encodedStr, this_fn, respond_with_user_reject);
#endif  // HAVE_BAGL
    }
...
void handlePoolFinancials_ui_runStep() {
    ...
    UI_STEP(HANDLE_POOL_FINANCIALS_STEP_DISPLAY_PLEDGE) {
        ...
        ui_getAdaAmountScreen(adaAmountStr, SIZEOF(adaAmountStr), subctx->stateData.pledge);

        fill_and_display_if_required("Pledge", adaAmountStr, this_fn, respond_with_user_reject);
#endif  // HAVE_BAGL
    }
    UI_STEP(HANDLE_POOL_FINANCIALS_STEP_DISPLAY_COST) {
#ifdef HAVE_BAGL
        ui_displayAdaAmountScreen("Cost", subctx->stateData.cost, this_fn);
#elif defined(HAVE_NBGL)
        char adaAmountStr[50] = {0};
        ui_getAdaAmountScreen(adaAmountStr, SIZEOF(adaAmountStr), subctx->stateData.cost);

        fill_and_display_if_required("Cost", adaAmountStr, this_fn, respond_with_user_reject);
#endif  // HAVE_BAGL
    }
    UI_STEP(HANDLE_POOL_FINANCIALS_STEP_DISPLAY_MARGIN) {
#ifdef HAVE_BAGL
        ui_displayPoolMarginScreen(subctx->stateData.marginNumerator,
                                   subctx->stateData.marginDenominator,
                                   this_fn);
#elif defined(HAVE_NBGL)
        char marginStr[20] = {0};
        ui_getPoolMarginScreen(marginStr,
                               SIZEOF(marginStr),
                               subctx->stateData.marginNumerator,
                               subctx->stateData.marginDenominator);
        fill_and_display_if_required("Profit margin", marginStr, this_fn, respond_with_user_reject);
#endif  // HAVE_BAGL
    }
...
void handlePoolRewardAccount_ui_runStep() {
    ...
    UI_STEP(HANDLE_POOL_REWARD_ACCOUNT_STEP_DISPLAY) {
#ifdef HAVE_BAGL
        ui_displayRewardAccountScreen(&subctx->stateData.poolRewardAccount,
        ...
        ui_getRewardAccountScreen(firstLine,
                                  ...
                                  commonTxData->networkId);
        fill_and_display_if_required(firstLine,
                                     secondLine,
...
void handleOwner_ui_runStep() {
    ...
    UI_STEP(HANDLE_OWNER_STEP_DISPLAY) {
#ifdef HAVE_BAGL
        ui_displayPoolOwnerScreen(&subctx->stateData.owner,
                                  subctx->currentOwner,
                                  commonTxData->networkId,
                                  this_fn);
#elif defined(HAVE_NBGL)
        char firstLine[32] = {0};
        char secondLine[BIP44_PATH_STRING_SIZE_MAX + MAX_HUMAN_REWARD_ACCOUNT_SIZE + 2] = {0};
        ui_getPoolOwnerScreen(firstLine,
                              SIZEOF(firstLine),
                              secondLine,
                              SIZEOF(secondLine),
                              &subctx->stateData.owner,
                              subctx->currentOwner,
                              commonTxData->networkId);
...
#ifdef HAVE_BAGL
        ui_displayPoolRelayScreen(relay, subctx->currentRelay, this_fn);
#elif defined(HAVE_NBGL)
        char line[20] = {0};
        ui_getPoolRelayScreen(line, SIZEOF(line), subctx->currentRelay);
        fill_and_display_if_required("Relay index", line, this_fn, respond_with_user_reject);
#endif  // HAVE_BAGL
    }
    UI_STEP(HANDLE_RELAY_IP_STEP_DISPLAY_IPV4) {
#ifdef HAVE_BAGL
        ui_displayIpv4Screen(&relay->ipv4, this_fn);
#elif defined(HAVE_NBGL)
        char ipStr[IPV4_STR_SIZE_MAX + 1] = {0};
        ui_getIpv4Screen(ipStr, SIZEOF(ipStr), &relay->ipv4);
        fill_and_display_if_required("IPv4 address", ipStr, this_fn, respond_with_user_reject);
#endif  // HAVE_BAGL
    }
    UI_STEP(HANDLE_RELAY_IP_STEP_DISPLAY_IPV6) {
#ifdef HAVE_BAGL
        ui_displayIpv6Screen(&relay->ipv6, this_fn);
#elif defined(HAVE_NBGL)
        char ipStr[IPV6_STR_SIZE_MAX + 1] = {0};
        ui_getIpv6Screen(ipStr, SIZEOF(ipStr), &relay->ipv6);
        fill_and_display_if_required("IPv6 address", ipStr, this_fn, respond_with_user_reject);
#endif  // HAVE_BAGL
    }
    UI_STEP(HANDLE_RELAY_IP_STEP_DISPLAY_PORT) {
#ifdef HAVE_BAGL
        ui_displayIpPortScreen(&relay->port, this_fn);
#elif defined(HAVE_NBGL)
        char portStr[1 + (sizeof "65536")] = {0};
        ui_getIpPortScreen(portStr, SIZEOF(portStr), &relay->port);
        fill_and_display_if_required("Port", portStr, this_fn, respond_with_user_reject);
#endif  // HAVE_BAGL
    }
...
#ifdef HAVE_BAGL
        ui_displayPaginatedText("DNS name", dnsNameStr, this_fn);
#elif defined(HAVE_NBGL)
        fill_and_display_if_required("DNS name", dnsNameStr, this_fn, respond_with_user_reject);
#endif  // HAVE_BAGL
    }
...
#ifdef HAVE_BAGL
        ui_displayPaginatedText("No metadata", "(anonymous pool)", this_fn);
#elif defined(HAVE_NBGL)
        fill_and_display_if_required("Metadata",
                                     "none (anonymous pool)",
                                     this_fn,
                                     respond_with_user_reject);
#endif  // HAVE_BAGL
...
void handleMetadata_ui_runStep() {
...
#ifdef HAVE_BAGL
        ui_displayPaginatedText("Pool metadata url", metadataUrlStr, this_fn);
#elif defined(HAVE_NBGL)
        fill_and_display_if_required("Pool metadata url",
                                     metadataUrlStr,
                                     this_fn,
                                     respond_with_user_reject);
#endif  // HAVE_BAGL
    ...
    UI_STEP(HANDLE_METADATA_STEP_DISPLAY_HASH) {
        ...
        size_t len =
            encode_hex(md->hash, SIZEOF(md->hash), metadataHashHex, SIZEOF(metadataHashHex));
        ASSERT(len + 1 == SIZEOF(metadataHashHex));

#ifdef HAVE_BAGL
        ui_displayPaginatedText("Pool metadata hash", metadataHashHex, this_fn);
#elif defined(HAVE_NBGL)
        fill_and_display_if_required("Pool metadata hash",
                                     metadataHashHex,
                                     this_fn,
                                     respond_with_user_reject);
#endif  // HAVE_BAGL
...
void signTxPoolRegistration_handleConfirm_ui_runStep() {
    ...
    UI_STEP(HANDLE_CONFIRM_STEP_FINAL_NO_OWNERS) {
        if (subctx->numOwners == 0) {
#ifdef HAVE_BAGL
            ui_displayPaginatedText("No", "pool owners", this_fn);
#elif defined(HAVE_NBGL)
            fill_and_display_if_required("Pool owners", "None", this_fn, respond_with_user_reject);
#endif  // HAVE_BAGL
        } else {
    ...
    UI_STEP(HANDLE_CONFIRM_STEP_FINAL_NO_RELAYS) {
        bool isOperator =
            commonTxData->txSigningMode == SIGN_TX_SIGNINGMODE_POOL_REGISTRATION_OPERATOR;
        if ((subctx->numRelays == 0) && isOperator) {
#ifdef HAVE_BAGL
            ui_displayPaginatedText("No", "pool relays", this_fn);
#elif defined(HAVE_NBGL)
            fill_and_display_if_required("Pool relays", "None", this_fn, respond_with_user_reject);
#endif  // HAVE_BAGL
        } else {
...
#ifdef HAVE_BAGL
        ui_displayPrompt("Confirm stake", "pool registration?", this_fn, respond_with_user_reject);
#elif defined(HAVE_NBGL)
        display_confirmation("Confirm stake pool\nregistration",
                             "",
                             "STAKE POOL\nREGISTERED",
                             "Stake pool\nrejected",
                             this_fn,
