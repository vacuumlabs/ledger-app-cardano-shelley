...
void test_assetFingerprint() {
    ...
    testcase_assetFingerprint("7eae28af2208be856f7a119668ae52a49b73725e326dc16579dcc373",
                              "",
                              "asset1rjklcrnsdzqp65wjgrg55sy9723kw09mlgvlc3");

    testcase_assetFingerprint("1e349c9bdea19fd6c147626a5260bc44b71635f398b67c59881df209",
...
const token_testcase_t tokenTestCases[] = {
    {
        {0x94, 0xcb, 0xb4, 0xfc, 0xbc, 0xaa, 0x29, 0x75, 0x77, 0x9f, 0x27, 0x3b, 0x26, 0x3e,
         0xb3, 0xb5, 0xf2, 0x4a, 0x99, 0x51, 0xe4, 0x46, 0xd6, 0xdc, 0x4c, 0x13, 0x58, 0x64},
        {0x52, 0x45, 0x56, 0x55},
        4,
        234,
    ...
    {// no decimal places in our table
     {0xaa, 0xcb, 0xb4, 0xfc, 0xbc, 0xaa, 0x29, 0x75, 0x77, 0x9f, 0x27, 0x3b, 0x26, 0x3e,
      0xb3, 0xb5, 0xf2, 0x4a, 0x99, 0x51, 0xe4, 0x46, 0xd6, 0xdc, 0x4c, 0x13, 0x58, 0x64},
     {0x52, 0x45, 0x56, 0x55},
     4,
     2345,
...
void test_decimalPlaces() {
    for (size_t i = 0; i < ARRAY_LEN(tokenTestCases); i++) {
        ...
        str_formatTokenAmountOutput(&group,
                                    tokenTestCases[i].assetNameBytes,
                                    tokenTestCases[i].assetNameSize,
                                    tokenTestCases[i].amountOutput,
                                    tokenAmountStr,
                                    SIZEOF(tokenAmountStr));
        EXPECT_EQ(strcmp(tokenAmountStr, tokenTestCases[i].expectedOutput), 0);

        str_formatTokenAmountMint(&group,
                                  tokenTestCases[i].assetNameBytes,
                                  tokenTestCases[i].assetNameSize,
