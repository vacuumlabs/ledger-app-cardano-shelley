...
void test_CIP15() {
...

    {
        ...
        auxDataHashBuilder_cVoteRegistration_addVoteKey(&builder, tmp, tmpSize);
    }

    {
        uint8_t tmp[32] = {0};
        size_t tmpSize = decode_hex(stakingKey, tmp, SIZEOF(tmp));
        auxDataHashBuilder_cVoteRegistration_addStakingKey(&builder, tmp, tmpSize);
    }
...

    {
        ...
        auxDataHashBuilder_cVoteRegistration_addSignature(&builder, tmp, tmpSize);
    }
