...
uint8_t getAddressType(uint8_t addressHeader)
{
	...
	return addressHeader & ADDRESS_TYPE_MASK;
}
...
static size_t view_appendVariableLengthUInt(write_view_t* view, blockchainIndex_t value)
{
	STATIC_ASSERT(sizeof(blockchainIndex_t) <= 8, "wrong blockchainIndex_t");
...
#define WRITE(fmt, ...) \
	...
	WRITE("(%d, ", (int) blockchainPointer.blockIndex);
	WRITE("%d, ", (int) blockchainPointer.txIndex);
	WRITE("%d)", (int) blockchainPointer.certificateIndex);
