...
__noinline_due_to_stack__ void signCVote_handleInitAPDU(const uint8_t* wireDataBuffer,
...

    {
        ...
        votecastHashBuilder_chunk(&ctx->votecastHashBuilder,
...
__noinline_due_to_stack__ void signCVote_handleConfirmAPDU(const uint8_t* wireDataBuffer
...

    {
        ...
        votecastHashBuilder_finalize(&ctx->votecastHashBuilder,
                                     ctx->votecastHash,
                                     SIZEOF(ctx->votecastHash));
    }
...
__noinline_due_to_stack__ void signCVote_handleWitnessAPDU(const uint8_t* wireDataBuffer,
...

    {
        ...
        size_t parsedSize =
            bip44_parseFromWire(&ctx->witnessData.path, wireDataBuffer, wireDataSize);
...

    {
        ...
        getWitness(&ctx->witnessData.path,
                   ctx->votecastHash,
                   SIZEOF(ctx->votecastHash),
                   ctx->witnessData.signature,
                   SIZEOF(ctx->witnessData.signature));
    }
