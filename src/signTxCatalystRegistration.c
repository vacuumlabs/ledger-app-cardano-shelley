...
static catalyst_registration_context_t* subctx = &(instructionState.signTxContext.stageContext.catalyst_registration_subctx);
static common_tx_data_t* commonTxData = &(instructionState.signTxContext.commonTxData);
static aux_data_hash_builder_t* auxDataHashBuilder = &(instructionState.signTxContext.stageContext.catalyst_registration_subctx.auxDataHashBuilder);
...
static void signTxCatalystRegistration_handleConfirm_ui_runStep()
{
	...
	UI_STEP(HANDLE_CONFIRM_STEP_DISPLAY_HASH) {
		ui_displayHexBufferScreen(
		        "Auxiliary data hash",
		        (uint8_t*) &subctx->stateData.confirmationResponse.auxDataHash,
		        SIZEOF(subctx->stateData.confirmationResponse.auxDataHash),
		        this_fn
		);
	}
	UI_STEP(HANDLE_CONFIRM_STEP_RESPOND) {
		io_send_buf(SUCCESS, (uint8_t*) &subctx->stateData.confirmationResponse, SIZEOF(subctx->stateData.confirmationResponse));
		advanceState();
	}
	...
	{
		{
			...
			auxDataHashBuilder_catalystRegistration_finalizePayload(auxDataHashBuilder, votingPayloadHashBuffer, METADATA_HASH_LENGTH);
			getCatalystVotingRegistrationSignature(
			        &subctx->stakingKeyPath,
			        votingPayloadHashBuffer, CATALYST_REGISTRATION_PAYLOAD_HASH_LENGTH,
			        (uint8_t*) &subctx->stateData.confirmationResponse.registrationSignature, ED25519_SIGNATURE_LENGTH
			);
		}
		auxDataHashBuilder_catalystRegistration_addSignature(auxDataHashBuilder, (uint8_t*) &subctx->stateData.confirmationResponse.registrationSignature, ED25519_SIGNATURE_LENGTH);
		auxDataHashBuilder_catalystRegistration_addAuxiliaryScripts(auxDataHashBuilder);

		auxDataHashBuilder_finalize(auxDataHashBuilder, (uint8_t*) &subctx->stateData.confirmationResponse.auxDataHash, METADATA_HASH_LENGTH);
