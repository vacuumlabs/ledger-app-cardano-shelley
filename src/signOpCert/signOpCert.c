...
uint16_t signOpCert_handleAPDU(uint8_t p1,
...

    {
        ...
        STATIC_ASSERT(SIZEOF(ctx->kesPublicKey) == KES_PUBLIC_KEY_LENGTH,
                      "wrong KES public key size");
        ...
        view_skipBytes(&view,
                       bip44_parseFromWire(&ctx->poolColdKeyPathSpec,
...

    {
        ...
        getOpCertSignature(&ctx->poolColdKeyPathSpec,
                           opCertBodyBuffer,
                           OP_CERT_BODY_LENGTH,
                           ctx->signature,
...
static void signOpCert_ui_runStep() {
    ...
    UI_STEP(UI_STEP_DISPLAY_POOL_COLD_KEY_PATH) {
        ...
        fill_and_display_if_required("Pool cold key path",
                                     pathStr,
                                     this_fn,
                                     respond_with_user_reject);
#endif  // HAVE_BAGL
    ...
    UI_STEP(UI_STEP_DISPLAY_KES_PUBLIC_KEY) {
#ifdef HAVE_BAGL
        ui_displayBech32Screen("KES public key",
                               "kes_vk",
                               ctx->kesPublicKey,
                               SIZEOF(ctx->kesPublicKey),
                               this_fn);
        ...
        ui_getBech32Screen(encodedStr,
                           ...
                           ctx->kesPublicKey,
                           SIZEOF(ctx->kesPublicKey));
        fill_and_display_if_required("KES public key",
                                     encodedStr,
                                     this_fn,
                                     respond_with_user_reject);
#endif  // HAVE_BAGL
...
#ifdef HAVE_BAGL
        ui_displayPaginatedText("KES period", kesPeriodString, this_fn);
#elif defined(HAVE_NBGL)
        fill_and_display_if_required("KES period",
                                     kesPeriodString,
                                     this_fn,
                                     respond_with_user_reject);
#endif  // HAVE_BAGL
...
#ifdef HAVE_BAGL
        ui_displayPaginatedText("Issue counter", issueCounterString, this_fn);
#elif defined(HAVE_NBGL)
        fill_and_display_if_required("Issue counter",
                                     issueCounterString,
                                     this_fn,
                                     respond_with_user_reject);
#endif  // HAVE_BAGL
...
#ifdef HAVE_BAGL
        ui_displayPrompt("Confirm", "operational certificate?", this_fn, respond_with_user_reject);
#elif defined(HAVE_NBGL)
