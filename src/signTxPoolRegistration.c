...
enum {
	...
	HANDLE_POOLPARAMS_STEP_DISPLAY_OPERATOR,
	HANDLE_POOLPARAMS_STEP_DISPLAY_VRF,
...
static void handlePoolParams_ui_runStep()
{
	...
	UI_STEP(HANDLE_POOLPARAMS_STEP_DISPLAY_OPERATOR) {
		ui_displayHexBufferScreen(
		        "Operator key hash",
		        subctx->poolParams.operatorHash, SIZEOF(subctx->poolParams.operatorHash),
		        this_fn
		);
	}
	UI_STEP(HANDLE_POOLPARAMS_STEP_DISPLAY_VRF) {
		ui_displayHexBufferScreen(
		        "VRF key hash",
		        subctx->poolParams.vrfKeyHash, SIZEOF(subctx->poolParams.vrfKeyHash),
		        this_fn
		);
	}
...
static void signTxPoolRegistration_handlePoolParamsAPDU(uint8_t* wireDataBuffer, size_t wireDataSize)
{
	...
	}
	{
...

		{
			...
			TRACE_BUFFER(wireHeader->operatorHash, SIZEOF(wireHeader->operatorHash));
			ASSERT(SIZEOF(wireHeader->operatorHash) == SIZEOF(p->operatorHash));
			os_memmove(p->operatorHash, wireHeader->operatorHash, SIZEOF(p->operatorHash));
...
static void handleOwner_ui_runStep()
{
	...
	UI_STEP(HANDLE_OWNER_STEP_DISPLAY) {
		// TODO FIXME add key derivation path as prefix to this hash
		ui_displayHexBufferScreen("Owner", subctx->owner.keyHash, SIZEOF(subctx->owner.keyHash), this_fn);
	}
...
static void signTxPoolRegistration_handleRelayAPDU(uint8_t* wireDataBuffer, size_t wireDataSize)
{
...

	{
		...
		case RELAY_SINGLE_HOST_IP: {
			...
			size_t ipv4Size = 0;
			{
				...
				if (includeIpv4 == RELAY_YES) {
					ipv4Size = 4;
					VALIDATE(view_remainingSize(&view) >= ipv4Size, ERR_INVALID_DATA);
					os_memmove(ipv4, view.ptr, ipv4Size);
					view_skipBytes(&view, ipv4Size);
					TRACE("ip v4");
					TRACE_BUFFER(ipv4, ipv4Size);
					ipv4Ptr = ipv4;
				} else {
			...
			size_t ipv6Size = 0;
			{
				...
				if (includeIpv6 == RELAY_YES) {
					ipv6Size = 16;
					VALIDATE(view_remainingSize(&view) >= ipv6Size, ERR_INVALID_DATA);
					os_memmove(ipv6, view.ptr, ipv6Size);
					view_skipBytes(&view, ipv6Size);
					TRACE("ip v6");
					TRACE_BUFFER(ipv6, ipv6Size);
					ipv6Ptr = ipv6;
				} else {
		...
		case RELAY_SINGLE_HOST_NAME: {
			...
			VALIDATE(view_remainingSize(&view) <= 64, ERR_INVALID_DATA);
			str_validateText(VIEW_REMAINING_TO_TUPLE_BUF_SIZE(&view));
...
enum {
	HANDLE_METADATA_STEP_DISPLAY_URL = 6340,
...
static void signTxPoolRegistration_handleConfirm_ui_runStep()
{
	...
	UI_STEP(HANDLE_CONFIRM_STEP_FINAL_CONFIRM) {
		ui_displayPrompt(
		        "Confirm stake pool",
		        "registration certificate?", // TODO FIXME does it fit on the screen?
		        this_fn,
