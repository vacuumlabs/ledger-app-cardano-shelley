...
static inline void advanceStage()
{
	...
	case SIGN_STAGE_METADATA:
		ctx->stage = SIGN_STAGE_CONFIRM; // TODO?
		break;
...
static void signTx_handleMetadataAPDU(uint8_t p2, uint8_t* wireDataBuffer, size_t wireDataSize)
{
	...
	ASSERT(wireDataSize < BUFFER_SIZE_PARANOIA);
	VALIDATE(wireDataSize >= 1, ERR_INVALID_DATA);
	uint8_t includeMetadata = wireDataBuffer[0];

	if (includeMetadata == SIGN_TX_METADATA_YES) {
		VALIDATE(wireDataSize == 1 + METADATA_HASH_LENGTH, ERR_INVALID_DATA);
		os_memcpy(ctx->metadataHash, wireDataBuffer + 1, METADATA_HASH_LENGTH);
