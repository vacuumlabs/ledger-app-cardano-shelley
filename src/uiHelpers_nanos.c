...
static unsigned int ui_busy_button(unsigned int button_mask MARK_UNUSED,
...
static void scroll_update_display_content() {
    ...
    memmove(ctx->currentText, ctx->fullText + ctx->scrollIndex, SIZEOF(ctx->currentText) - 1);
    UX_REDISPLAY();
}
...
unsigned int ui_prompt_button(unsigned int button_mask,
                              unsigned int button_mask_counter MARK_UNUSED) {
    TRY_CATCH_UI({
...
static const bagl_element_t* ui_prepro_paginatedText(const bagl_element_t* element) {
    ...
    switch (element->component.userid) {
        case ID_ICON_GO_LEFT:
            return (ctx->scrollIndex != 0 || textFitsSinglePage) ? element : NULL;
        case ID_ICON_GO_RIGHT:
            return ((ctx->scrollIndex + SIZEOF(ctx->currentText) < strlen(ctx->fullText) + 1) ||
                    textFitsSinglePage)
                       ? element
                       : NULL;
        default:
