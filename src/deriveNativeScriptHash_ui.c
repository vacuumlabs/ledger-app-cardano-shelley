...
#ifdef HAVE_BAGL
    ui_displayPaginatedText(HEADER, positionDescription, callback);
#elif defined(HAVE_NBGL)
...
static void deriveScriptHash_display_ui_runStep_cb(void) {
    ...
    fill_and_display_if_required("Script contents",
                                 text,
    ...
    switch (ctx->ui_scriptType) {
...
#ifdef HAVE_BAGL
            ui_displayPathScreen(HEADER, &ctx->scriptContent.pubkeyPath, this_fn);
#elif defined(HAVE_NBGL)
        ...
        case UI_SCRIPT_PUBKEY_HASH: {
            ...
            ui_getBech32Screen(encodedStr,
                               ...
                               ctx->scriptContent.pubkeyHash,
                               ADDRESS_KEY_HASH_LENGTH);
            fill_and_display_if_required("Pubkey hash",
                                         encodedStr,
                                         this_fn,
                                         respond_with_user_reject);
#endif  // HAVE_BAGL
        ...
        case UI_SCRIPT_ANY: {
            ...
            ASSERT(ctx->level < MAX_SCRIPT_DEPTH);
            STATIC_ASSERT(
                sizeof(ctx->complexScripts[ctx->level].remainingScripts) <= sizeof(unsigned),
                "oversized type for %u");
            STATIC_ASSERT(!IS_SIGNED(ctx->complexScripts[ctx->level].remainingScripts),
                          "signed type for %u");
#ifdef HAVE_BAGL
            snprintf(text,
                     SIZEOF(text),
                     "Contains %u nested scripts.",
                     ctx->complexScripts[ctx->level].remainingScripts);
            // make sure all the information is displayed to the user
            ASSERT(strlen(text) + 1 < SIZEOF(text));

            ui_displayPaginatedText(HEADER, text, this_fn);
#elif defined(HAVE_NBGL)
            snprintf(text,
                     SIZEOF(text),
                     "%u nested scripts",
        ...
        case UI_SCRIPT_N_OF_K: {
            ASSERT(ctx->level < MAX_SCRIPT_DEPTH);
            STATIC_ASSERT(sizeof(ctx->scriptContent.requiredScripts) <= sizeof(unsigned),
                          "oversized type for %u");
            ...
            snprintf(text,
                     SIZEOF(text),
                     "Requires %u out of %u signatures. Contains %u nested scripts",
            ...
            // make sure all the information is displayed to the user
            ASSERT(strlen(text) + 1 < SIZEOF(text));
            fill_and_display_if_required("Requirement",
                                         text,
                                         deriveScriptHash_display_ui_runStep_cb,
                                         respond_with_user_reject);
#endif  // HAVE_BAGL
            break;
        }
...
#ifdef HAVE_BAGL
            ui_displayUint64Screen(HEADER, ctx->scriptContent.timelock, this_fn);
#elif defined(HAVE_NBGL)
            char line[30];
            ui_getUint64Screen(line, SIZEOF(line), ctx->scriptContent.timelock);
        ...
        case UI_SCRIPT_INVALID_HEREAFTER: {
            ...
            char line[30];
            ui_getUint64Screen(line, SIZEOF(line), ctx->scriptContent.timelock);
            fill_and_display_if_required("Invalid hereafter",
                                         line,
                                         this_fn,
                                         respond_with_user_reject);
#endif  // HAVE_BAGL
...
void deriveScriptHash_display_ui_runStep() {
    ...
    UI_STEP(DISPLAY_UI_STEP_SCRIPT_TYPE) {
        ...
        switch (ctx->ui_scriptType) {
            case UI_SCRIPT_PUBKEY_PATH:
                fill_and_display_if_required("Script type",
                                             "Pubkey path",
                                             this_fn,
                                             respond_with_user_reject);
                break;
...
static void deriveNativeScriptHash_displayNativeScriptHash_finish(void) {
    display_confirmation("Confirm script",
                         "",
                         "SCRIPT\nCONFIRMED",
                         "Script\nrejected",
...
void deriveNativeScriptHash_displayNativeScriptHash_bech32() {
#ifdef HAVE_BAGL
    ui_displayBech32Screen("Script hash",
                           "script",
    ...
    ui_getBech32Screen(encodedStr,
                       ...
                       ctx->scriptHashBuffer,
                       SCRIPT_HASH_LENGTH);
    fill_and_display_if_required("Script hash",
                                 encodedStr,
                                 deriveNativeScriptHash_displayNativeScriptHash_finish,
                                 respond_with_user_reject);
#endif  // HAVE_BAGL
}
...
void deriveNativeScriptHash_displayNativeScriptHash_policyId() {
#ifdef HAVE_BAGL
    ui_displayHexBufferScreen("Policy ID",
                              ctx->scriptHashBuffer,
                              SCRIPT_HASH_LENGTH,
                              deriveNativeScriptHash_displayNativeScriptHash_callback);
#elif defined(HAVE_NBGL)
    ...
    fill_and_display_if_required("Policy ID",
                                 bufferHex,
                                 deriveNativeScriptHash_displayNativeScriptHash_finish,
                                 respond_with_user_reject);
#endif  // HAVE_BAGL
